...
<body>
      ...
      <div class="row">
        <div class="table">
          <div class="table-body table-responsive">
            <table id="bookTable" class="display  table-hover table-bordered table-sm" cellspacing="0" width="100%">
              <thead>
                <tr>
                  <th>رقم الكتاب</th>
                  <th>اسم الكتاب</th>
                  <th>الاسم بالإنجليزية</th>
                  <th class="none">الوصف بالعربية</th>
                  <th class="none">الوصف بالإنجليزية</th>
                  <th>الإجراءات</th>
                </tr>
              </thead>
              <tbody>
  ...
  <div id="modalBackground" class="modal-background">
    <div id="modal" class="modal">
      ...
      <!-- Form -->
      <form id="saveBookForm" class="form" method="post">
        <div class="row">
          <div class="full-width col-lg-12">
            <div class="input-field">
              <label for="collection-Select">اختر المجموعة</label>
              <select id="collection-Select" name="collection-Select">
                <option value=""></option>
              </select>
            </div>
          </div>
          <div class="col-6">
            <div class="input-field">
              <input placeholder="اسم الكتاب بالعربية" type="text" class="input" name="book_name_ab" id="book_name_ab" autocomplete="off" autocapitalize="on" required />
            </div>
          </div>
          <div class="col-6">
            <div class="input-field">
              <input placeholder="اسم الكتاب بالإنجليزية" type="text" class="input" name="book_name_en" id="book_name_en" autocomplete="off" autocapitalize="on" required />
            </div>
          </div>
          <div class="full-width col-lg-12">
            <div class="input-field">
              <textarea placeholder="الوصف بالعربية" class="input col-12" name="DesBookAb" placeholder="" required id="DesBookAb"></textarea>
            </div>
          </div>
          <div class="full-width col-lg-12">
            <div class="input-field">
              <textarea placeholder="الوصف بالإنجليزية" class="input col-12" name="DesBookEn" placeholder="" required id="DesBookEn"></textarea>
            </div>
          </div>
          <div class="full-width col-lg-12">
            <div class="input-field">
  ...
  <script>
    $(document).ready(function() {
      // Make AJAX request to fetch collections
      $.ajax({
        url: 'get_collection.php',
        type: 'GET',
        success: function(response) {
          // Update the select elements with fetched collections
          $('#collectionSelect').html(response);
          $('#collection-Select').html(response);
        },
  ...
  <script type="text/javascript">
    $(document).ready(function() {
      ...
      var dataTable = $("#bookTable").DataTable({
        ...
        'ajax': {
          'url': 'fetch_book_data.php',
          'type': 'post',
          'data': function(data) {
            data.collectionID = $('#collectionSelect').val();
          }
        },
        'fnCreatedRow': function(nRow, aData, iDataIndex) {
          $(nRow).attr('BookID', aData[0]);
        },
      });

      // Change event listener for the select element
      $('#collectionSelect').on('change', function() {
        // Trigger DataTable's AJAX reload
    ...
    $('#saveBookForm').submit(function(event) {
      ...
      $.ajax({
        ...
        success: function(response) {
          ...
          // Reload the datatable after adding a new book
