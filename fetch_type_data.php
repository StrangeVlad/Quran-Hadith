...
$countSql = "SELECT COUNT(*) AS total FROM types";

if (isset($_POST['search']['value'])) {
  $search_value = $_POST['search']['value'];
  $sql .= " WHERE (typ_name LIKE '%" . $search_value . "%'";
  $sql .= " OR description LIKE '%" . $search_value . "%')";

  $countSql .= " WHERE (typ_name LIKE '%" . $search_value . "%'";
...
if (isset($_POST['order'])) {
  ...
  $columns = array('typ_name', 'description');
...
if ($query && $countQuery) {
  ...
  while ($row = mysqli_fetch_assoc($query)) {
    $subarray = array();
    $subarray[] = $row['TypeID'];
    $subarray[] = $row['typ_name'];
    $subarray[] = $row['description'];
    $subarray[] = '<a href="javascript:void();" data-id="' . $row['TypeID'] . '" class="btn btn-sm btn-danger btnDelete">حذف</a>';
    $data[] = $subarray;
  }
